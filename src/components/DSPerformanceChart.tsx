...
import { DSPerformance } from '../types';

interface DSPerformanceChartProps {
  data: DSPerformance[];
}

export const DSPerformanceChart: React.FC<DSPerformanceChartProps> = ({ data }) => {
  const chartData = data.map(item => ({
    name: item.dsName.replace('Dr. ', ''),
    fullName: item.dsName,
    state: item.state,
    reviewed: item.totalReviewed,
    approved: item.totalApproved,
    rejected: item.totalRejected,
    sentBack: item.totalSentBack,
        ...
        <div className="bg-white p-4 rounded-lg shadow-lg border border-gray-200">
          ...
          <div className="space-y-1">
            ...
            <p className="text-orange-600">
              Sent Back: <span className="font-semibold">{data.sentBack.toLocaleString()}</span>
            </p>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
      <h3 className="text-lg font-semibold text-gray-900 mb-6">DS's Data Review Performance</h3>
      <ResponsiveContainer width="100%" height={400}>
        <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
          ...
          <Bar dataKey="sentBack" fill="#F97316" name="Sent Back" radius={[2, 2, 0, 0]} />
